...
var verifyPassword = function(username, candidatepw, callback) {
  UserAuth.find({username:username}, function(err,results){
    if(results.length > 0){
        var user = results[0];
...
var createNewUser = function (username, password, callback) {
  UserAuth.find({username:username}, function(err,results){
    if(results.length>0){
      ...
      UserAuth.create({'username':username,
                   'password':password}, function(err,user){callback(null,user)});
    }
  });
};
...
router.post('/login', function(req, res) {
  ...
  verifyPassword(req.body.username, req.body.password, function(err, match) {
    if (match) {
